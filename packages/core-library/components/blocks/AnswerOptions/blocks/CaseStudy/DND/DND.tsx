...
export const DND: React.FC<DNDPropsType> = ({ questionIndex }) => {
  ...
  const { createDndOption, optionList, deleteDndOption } = useDndOptions(
    questionIndex,
    formId ?? ""
  );
  const {
    control: createDndControl,
    handleSubmit: handleSubmitOption,
    reset: resetOptionField,
  } = useForm<CreateDndOptionType>({
  ...
  return (
    <Box data-testid="dnd-form">
      <Card sx={{ minHeight: "400px", overflow: "auto", px: 2 }}>
        ...
        <DNDOptionList
          deleteDndOption={deleteDndOption}
          optionList={optionList}
        />
      </Card>
      <Box sx={{ p: 5 }}>
