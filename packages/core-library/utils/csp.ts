...
export const generateCSP = (generatedNonce: string): string =>
  `default-src 'self' *.vercel.app; script-src 'self' 'nonce-${generatedNonce}' 'unsafe-eval' https://js.stripe.com *.vercel.app *.herokuapp.com https://vercel.live https://www.google.com ` +
  ...
  " *.vercel.app *.herokuapp.com https://js.stripe.com https://api.ipify.org https://www.google.com " +
  config.value.STRIPE_URL_JS +
  ` blob:; img-src 'self' data: blob: webpack:; font-src 'self' data: https://fonts.gstatic.com; frame-src 'self' *.vercel.app https://js.stripe.com https://vercel.live ` +
  " " +
  config.value.STRIPE_URL_JS +
  ";";
