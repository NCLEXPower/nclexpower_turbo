...
import { ChatBotSsr, CmsGlobals, GoLiveStatusSsr, MaintenanceSsr } from "./types/global";
...
export async function getHasActiveGoLive(clientCountry : string) {
  const response = await fetch(
    `${baseUrl}/api/v2/internal/baseInternal/active-schedule?${qs.stringify({ clientCountry })}`,
    {
      method: "GET",
      headers: headers,
    }
