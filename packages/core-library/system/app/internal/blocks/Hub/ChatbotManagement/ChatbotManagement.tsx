...
export const ChatbotManagement: React.FC<Props> = ({ onSubmit }) => {
  ...
  const { data: subsequentDataLists } = businessQueryGetSubsequentList([
    "subsequentLists",
  ]);
      ...
      >
        ...
        <div className="flex flex-col">
          ...
          <Typography sx={{ color: "#3B0086" }}>
            This is the chatbot management page user interface
          </Typography>
        </div>
      </Box>
      <Grid container sx={{ width: "100%", display: "flex", height: "100%" }}>
        <Grid lg={3} sx={{ paddingX: 1 }} item>
          <Button
            sx={{
              width: "100%",
              backgroundColor: "rgba(59, 0, 134, 0.05)",
        ...
        <Grid lg={6} item sx={{ paddingX: 1 }}>
          ...
          >
            ...
            <Box sx={{ width: "100%" }}>
              <Typography
                sx={{ color: "#3B0086", fontWeight: "bold", marginBottom: 2 }}
              >
                Options:{" "}
              </Typography>
        ...
        <Grid lg={3} item sx={{ paddingX: 1 }}>
          ...
          >
            <Typography
              sx={{ color: "#3B0086", fontWeight: "bold", padding: 4 }}
            >
              Subsequent Lists:
            </Typography>
            <hr />
            {subsequentDataLists && subsequentDataLists.length > 0 ? (
              subsequentDataLists?.map((item) => (
                <Box
                  key={item.id}
                  sx={{
                    marginBottom: 2,
              ...
              <Typography sx={{ color: "#3B0086", padding: 4 }}>
                No subsequent lists available
              </Typography>
            )}
