...
export const InclusionBlock: React.FC<Props> = ({ data }) => {
  ...
  const { businessQueryGetAllInclusion, businessQueryCreateInclusion } =
    useBusinessQueryContext();
  const {
    data: inclusionLists,
    isLoading,
    refetch,
  } = businessQueryGetAllInclusion(["getAllInclusionApi"]);
  ...
  async function onDelete(id: string) {
    try {
      ...
      console.error(error);
      executeToast(
        `Something went wrong during deletion ${error}. Please try again later`,
        "top-right",
        true,
        { type: "error" }
      );
          ...
          return (
            <Box display="flex" alignItems="center" height={1}>
              ...
              >
                ...
                <ListItemButton sx={{ color: "black" }} onClick={() => open()}>
                  Delete
                </ListItemButton>
                <ConfirmationDeleteDialog
                  isOpen={props.isOpen}
  ...
  async function onSubmit(value: InclusionType) {
    try {
      ...
      console.error(error);
      executeToast(
        `Something went wrong during submission ${error}. Please try again later`,
        "top-right",
        true,
        { type: "error" }
      );
